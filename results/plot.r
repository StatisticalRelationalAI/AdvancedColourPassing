...
if (file.exists(file_inter)) {
  ...
  for (i in c(3,5,10,15)) {
    ...
    if (use_tikz) {
      tikz(paste("plot-results-inter-p=", i, ".tex", sep=""), standAlone = FALSE, width = 3.3, height = 1.6)
    } else {
      pdf(file = paste("results-inter-p=", i, ".pdf", sep=""), height = 2.4)
    }
...
if (file.exists(file_intra)) {
  ...
  for (i in c(1,3,7)) {
    ...
    if (use_tikz) {
      tikz(paste("plot-results-intra-k=", i, ".tex", sep=""), standAlone = FALSE, width = 3.3, height = 1.6)
    } else {
      pdf(file = paste("results-intra-k=", i, ".pdf", sep=""), height = 2.4)
    }
