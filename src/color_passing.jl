...
function color_passing(
	...
	initcolors_cp!(node_colors, factor_colors, fg)
	...
	while true
		...
		changed |= assigncolors_cp!(factor_colors, f_signatures, fg)
		...
		changed |= assigncolors_cp!(node_colors, rv_signatures, fg)
...
"""
	initcolors_cp!(node_colors::Dict{RandVar, Int}, factor_colors::Dict{Factor, Int}, fg::FactorGraph)

Initialize the color dictionaries `node_colors` and `factor_colors` for the
factor graph `fg`.
"""
function initcolors_cp!(
	node_colors::Dict{RandVar, Int},
	factor_colors::Dict{Factor, Int},
	fg::FactorGraph
)
	assigncolors_cp!(node_colors, Dict{RandVar, Vector{Tuple{Int, Int}}}(), fg)
	assigncolors_cp!(factor_colors, Dict{Factor, Vector{Int}}(), fg)
end

"""
	assigncolors_cp!(node_colors::Dict{RandVar, Int}, rv_signatures::Dict{RandVar, Vector{Tuple{Int, Int}}}, fg::FactorGraph)::Bool

Re-assign colors to the random variables in `fg` based on the signatures
`rv_signatures`.
"""
function assigncolors_cp!(
	node_colors::Dict{RandVar, Int},
	rv_signatures::Dict{RandVar, Vector{Tuple{Int, Int}}},
	fg::FactorGraph
)::Bool
...
"""
	assigncolors_cp!(factor_colors::Dict{Factor, Int}, f_signatures::Dict{Factor, Vector{Int}}, fg::FactorGraph)::Bool
